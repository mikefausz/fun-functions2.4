...
// [c]: z then y
//
// Although letter is set to y and logged in the code before z,
// setTimeout() delays its execution by a millisecond, so z is
// logged first. x is never logged because letter is reset before
// any console logs.
//
// -----------------------------------------------
...
var spanishColor = function(colorName) {
  return hexColors[colorName.toLowerCase()];
}
...
// Object pattern - can still access score if you can find it
var scoreObj = {
  score: 0,
  getScore: function () {
    return this.score;
...
// Module pattern - cannot access score outside module
...
// Put your answer below -------------------------
//
// If you call the function without a parameter
// amount is undefined. speed += undefined is NaN.
...
var callLater = function(timeout, callback) {
  if (isNaN(timeout)) {
    setTimeout(callback, 1000); // default = 1s
  }
  else {
    setTimeout(callback, timeout);
  }
